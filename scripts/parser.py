...
class ConfigParser:
        ...
        try:
            ...
            if '@' in uri_part:
                ...
                try:
                    ...
                    method, password = cred_part.split(':', 1) if ':' in cred_part else (cred_part, '')
    ...
    def rebuild_uri(node: VPNNode, new_remark: Optional[str] = None) -> str:
        ...
        remark = new_remark if new_remark else node.remark
        
        if node.protocol == 'vless':
            params = '&'.join(f"{k}={v}" for k, v in node.extra.items() if k not in ['uuid'])
